...
atlas_dir_list = [d for d in natsorted(glob(ROOT_DIR+"/data/atlases/Mindboggle_atlases/*")) if os.path.isdir(d)]

# loop over all atlases, predict with all other atlases
for i in range(len(atlas_dir_list)):
    img_path = os.path.join(atlas_dir_list[i], 'orig_mni_aligned.nii.gz')
    mask_path = None  # these atlases don't have masks

    results_dir = ROOT_DIR+"/data/results/results_Mindboggle_atlases_leaveoneout/" + os.path.basename(atlas_dir_list[i])
