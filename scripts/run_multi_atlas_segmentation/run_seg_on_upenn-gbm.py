...
img_paths = natsorted(glob(os.path.join(ROOT_DIR, "data", "input", "UPENN-GBM", "UPENN-GBM-*", "*_T1_unstripped.nii.gz")))
mask_paths = natsorted(glob(os.path.join(ROOT_DIR, "data", "input", "UPENN-GBM", "UPENN-GBM-*", "*_mask.nii.gz")))

assert(len(img_paths) == len(mask_paths)), f"Number of images ({len(img_paths)}) and masks ({len(mask_paths)}) do not match"


img_path = img_paths[taskid]
mask_path = mask_paths[taskid]

atlas_dir_list = [d for d in glob(ROOT_DIR+"/data/atlases/Mindboggle_atlases/*") if os.path.isdir(d)]
results_dir = os.path.join(ROOT_DIR, "data/results/results_Mindboggle_on_UPENN-GBM",  os.path.basename(os.path.dirname(img_path)))
